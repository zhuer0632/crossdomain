...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				//document.addCookie("${cookie}");
			    var _frm = document.createElement("iframe"); 
			    _frm.style.display="block";
			    _frm.src="http://xj.gldjc.com/info_prices/22145/trend?${cookie}"; document.body.appendChild(_frm);   
			});
		</script>
	</head>
	<body>
	...
	<div>
		...
		当前网站首页是：
		<a href="http://192.168.1.215:8080/xj/adminCom/listComMgrtoAudit.do" target="_blank" >215 xj中的受限页面</a> 
        ...
        <div>
         注册<br>
			<form action="http://xj.gldjc.com/api/users/reg" method="post" >
					昵称 <input name="nickname" value="zhushaolong_123"  /> 必须存在且不能重复[字母开头4-16个英文字母或下划线]
					<br/>
				    密码 <input name="password" value="密码"  />    不限制
					<br/>
					确认 <input name="password_confirmation" value="确认密码"  /> 不限制
					<br/>
					邮箱 <input name="email" value="邮箱"  />  必须存在且不能重复
					<br/>
					名称 <input name="name" value="名称"  />  不限制
					<br/>
                    手机 <input name="mobile_phone" value="18610838555"  /> 必须存在且不能重复
					<br/>
                    公司 <input name="company_name" value="ccccc"  /> 不限制
					<br/>                   
					<input type="submit" value="提交" /> 
			</form>
		</div>
    ...
    // /^[a-zA-Z][a-zA-Z0-9_]{3,15}$/, :if => Proc.new {|x| x.new_record? }<br>
